...
struct ChatViewModelTests {
    ...
    func actionMajorClassification() {
        ...
        #expect(AIAction.createMilestone(phaseId: UUID(), name: "test", deadline: nil).isMajor == true)
        #expect(AIAction.createTask(milestoneId: UUID(), name: "test", priority: .normal, effortType: nil, deadline: nil).isMajor == true)
        #expect(AIAction.updateProject(projectId: UUID(), fields: ["notes": "test"]).isMajor == true)
        #expect(AIAction.updateTask(taskId: UUID(), fields: ["name": "test"]).isMajor == true)
